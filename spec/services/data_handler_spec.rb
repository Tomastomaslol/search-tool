...
describe DataHandler do
  ...
  context 'Given that feature sligthly smarter search is true' do
    context 'Given valid data' do
      before do
        ...
        allow_any_instance_of(described_class).to receive(:SLIGTHLY_SMARTER_SEARCH) { true }
      end
      ...
      describe '#search_for_value' do
        ...
        it 'returns a search result if there is a matching tag' do
          expect(subject.search_for_value('tags', 'lamp')).to eq [example_response[0]]
        end

        it 'returns 2 search results if there is a matching tag' do
          expect(subject.search_for_value('tags', 'desk')).to eq [example_response[0], example_response[1]]
        end

        it 'returns 2 search results if there is 2 partially matching tag' do
          expect(subject.search_for_value('tags', 'des')).to eq [example_response[0], example_response[1]]
        end

        it 'returns 0 search results if there is no matching tag' do
          expect(subject.search_for_value('tags', 'there is no match for this tag')).to eq []
        end
      end
    end
  ...
  context 'Given that feature sligthly smarter search is false' do
    context 'Given valid data' do
      before do
        allow_any_instance_of(described_class).to receive(:parse_read_file) { example_response }
        allow_any_instance_of(described_class).to receive(:read_file) { '' }
        allow_any_instance_of(described_class).to receive(:SLIGTHLY_SMARTER_SEARCH) { false }
      end
